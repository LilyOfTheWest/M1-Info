...
void communication(int val, int *tlect, int *tecr);
...
/* On récupère les fd des tubes */
void selection_tubes(int n, int** tubes, int *tlect, int *tecr) {
	int i, ecr, lect;

	/* On récupère les bons fd de chaque processus */
	if (rang == 0) {
		*tlect = tubes[n-1][0];
		*tecr = tubes[0][1];
...
int tirage_aleatoire() {
	...
	/* On affiche la valeur de ce processus */
