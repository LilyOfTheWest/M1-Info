...
int compacter_ff(int t, liste *l_occupe, liste *l_libre){
    ...
    printf("Compactage...\n\n");
    return allouer_ff(t, &(*l_libre), l_occupe, 0);
}
...
int supprime_occupe_ff(int t, liste *l_occupe, liste *l_libre){
    ...
    //sinon si la première cellule est la bonne
        ...
        it_libre = liberer_ff(it_occupe->adresse, it_occupe->taille, l_libre);
        ...
        *l_occupe = it_occupe->suivant;
        ...
        if(it_occupe->suivant == NULL){ // on a pas trouvé le programme avec la taille spécifiée, on retourne 0
            ...
            it_libre = liberer_ff((it_occupe->suivant)->adresse, (it_occupe->suivant)->taille, l_libre);
...
int allouer_ff(int t, liste *l_libre, liste *l_occupe, int c){
    ...
    if(it_libre == NULL){ 
        ...
        //si le suivant est nul on a pas trouvé de zone libre suffisement grande
        if(it_libre->suivant == NULL){
            //si compactage est activé, on appelle la fonction compacter
            if(c == 1){
                return compacter_ff(t, l_occupe, l_libre);
            }
...
//fonction permettant d'afficher les listes occupée et libre en firstfit
