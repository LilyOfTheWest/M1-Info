...
int allouer_nf(int t, liste *l_libre, liste *l_occupe, int c){
    static liste it_libre = NULL;
    liste it_libre2;
    liste save_it_libre;
    liste fin;

    //si la variable static est nulle ou qu'elle a une valeur importante, on la remet à jour au début de la liste libre
    if(it_libre == NULL || it_libre->adresse > 2000)
        it_libre = *l_libre;

    save_it_libre = it_libre; //on prend une sauvegarde de la fleche à l'état initial

    fprintf(stderr, "la fleche vaut %d et la sauvegarde vaut %d\n", it_libre->adresse, save_it_libre->adresse);
    
    if(it_libre == NULL){ // si on a pas de liste libre alors on retourne 0 car on ne peut rien allouer
        return 0;
    }
    else if(it_libre->taille >= t){ //si le premier convient, on insere un processus occupé à l'adresse voulue
        ...
        if(it_libre->taille == 0){
            if((it_libre->precedent)->adresse > it_libre->adresse){ // alors c'était le premier de la liste circulaire, il faut maj *l_libre
                it_libre2 = *l_libre;

                (it_libre->precedent)->suivant = it_libre->suivant;
                (it_libre->suivant)->precedent = it_libre->precedent;
                *l_libre = it_libre->suivant;

                free(it_libre2);

                it_libre = *l_libre; //on maj notre fleche
            }
            else if((it_libre->precedent)->adresse == it_libre->adresse){ // alors la liste libre contenait qu'un singleton, il faut mettre à jour *l_libre à NULL car elle ne contiendra plus de zone libre
                free(*l_libre);
                *l_libre = NULL;
                it_libre = NULL; //notre fleche ne pointe plus sur rien
            }
                ...
                it_libre = it_libre->suivant; //on met à jour notre fleche
        ...
        if(it_libre->suivant == fin){
            ...
            it_libre = it_libre->suivant; // on met notre fleche sur le noeud qui nous interresse
            ...
            if(it_libre->taille == 0){
                if((it_libre->precedent)->adresse > it_libre->adresse){ // alors c'était le premier de la liste circulaire, il faut maj *l_libre
                    ...
                    *l_libre = it_libre->suivant;
                    ...
                    it_libre = it_libre->suivant; //on maj notre fleche
                    
                    free(it_libre2);

                }
                else if((it_libre->precedent)->adresse == it_libre->adresse){ 
                    free(*l_libre);
                    *l_libre = NULL;
                    it_libre = NULL; //notre fleche ne pointe plus sur rien
                }
