...
%AFFICHAGE 
draw_fig_hist_black_and_white(ima_RH,['image réhaussée, alpha = ' num2str(alpha)])

%Troncature entre 0 et 255
ima_RH_tronc = min(max(ima_RH, 0), 255); 

%AFFICHAGE 
draw_fig_hist_black_and_white(ima_RH_tronc,['image réhaussée, alpha = ' num2str(alpha)])
...
ima_RH2_tronc = min(max(ima_RH2, 0), 255); 
%AFFICHAGE 
draw_fig_hist_black_and_white(ima_RH2_tronc,['image réhaussée 2 fois, alpha = ' num2str(alpha)])
